...
public class ProChecker {
  ...
  public static ProState getProState(Context context) {
    ...
    int versionCode = 0;
    int sigMatch = packageManager.checkSignatures(PRO_PACKAGE_NAME, context.getPackageName());
    try {
      versionCode = packageManager.getPackageInfo(PRO_PACKAGE_NAME, 0).versionCode;
